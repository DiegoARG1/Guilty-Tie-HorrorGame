...
vec4 CalcAllLights()
{
     ...
     // CORRECCIÓN 1: Inicializamos en negro absoluto para evitar basura matemática
...
vec4 CalcPointLight(PointLight light, vec3 normal, vec3 fragPos, vec3 viewDir)
{
    ...
    // CORRECCIÓN 2: Eliminado el + (color_final * 0.2) que causaba el 80% de brillo extra
    return (ambient + diffuse + specular);
}
...
vec4 CalcSpotLight(SpotLight light, vec3 normal, vec3 fragPos, vec3 viewDir)
{
   ...
   vec4 ambient, diffuse, specular;
    ...
    // CORRECCIÓN 3: Eliminado el + (color_final * 0.1) que causaba el 40% de brillo extra
    return (ambient + diffuse + specular); 
}
