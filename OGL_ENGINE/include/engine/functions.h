...
void processInput(GLFWwindow *window)
{
    ...
    else
    {
        ...
        {
            if (!teclaFPulsada) // Si no la estábamos presionando antes
            {
                linternaEncendida = !linternaEncendida; // Invertimos el estado (On/Off)
                teclaFPulsada = true; // Bloqueamos para que no parpadee súper rápido
            }
        }
        else if (glfwGetKey(window, GLFW_KEY_F) == GLFW_RELEASE)
        {
            teclaFPulsada = false; // Liberamos la tecla para el próximo clic
        }
...
void collidedObject_callback(string nameCollidedObject)
{
    ...
    if (nameCollidedObject == "Anakin" && control_salud )
    {
        choco = true;
        salud -=50;
        control_salud = false;
    ...
    if (nameCollidedObject == "carrorojo" && control_salud)
    {
        choco = true;
        salud -= 50;
        control_salud = false;
    ...
    if (nameCollidedObject == "switch_luz")
    {
        camera.Position = posicion_2;
        posicion_y = 100.0f;
        mainLight =glm::vec3(0.2f);
    }

    if (nameCollidedObject == "trampa")
    {
        trampa_move = true;
        mainLight.x = 0.7;
        mainLight.y = 0.0;
    ...
    if (nameCollidedObject == "Torre" )
    {
        choco_torre = true;
     
    }
    
    if (nameCollidedObject == "obstaculo_salida")
    {
        posicion_suelo = 3.0f;
    }
...
void collidedObject_callback(string nameCollidedObject, string nameCollidedObject2)
{
    ...
    cout << nameCollidedObject + " " + nameCollidedObject2 << endl;

    /*if (nameCollidedObject == "carroazul" && nameCollidedObject2 == "Anakin") {
        choco = true;
        sonar_efecto1 = true;
    }*/
